...
def ae_resnet_save(file_path, file_name, ae_resnet, optimizer=None):
    state_dict = {
        'net_arch': 'ae_resnet',
        'model': ae_resnet.state_dict(),
    }
...
def ae_resnet_load(file_path, file_name, model, optimizer=None):
    ...
    assert ('net_arch' in keys) and (
        'model' in keys), 'Cannot read this file in address : {}/{}.pth'.format(file_path, file_name)
    assert check_points['net_arch'] == 'ae_resnet', 'This file model architecture is not \'ae_resnet\''
...
def ae_resnet_accuracy(predicted_output, expected_output):
...
def ae_resnet_ae_rate_scheduler(ae_rate_max, curr_epoch, num_epoch):
    new_ae_rate = -(ae_rate_max/num_epoch)*curr_epoch + ae_rate_max
    return new_ae_rate if new_ae_rate > 0 else 0.
...
def ae_resnet_train(
    ...
    ae_rate_max = None
    if device == 'multi':
        ae_rate_max = ae_resnet.module.ae_alpha
    else:
        ae_rate_max = ae_resnet.ae_alpha
    ...
    for epoch in range(start_epoch, start_epoch+num_epoch):
        ...
        new_ae_rate = ae_resnet_ae_rate_scheduler(
            ae_rate_max, epoch, num_epoch)
        if device == 'multi':
            ae_resnet.module.ae_alpha = new_ae_rate
        else:
            ae_resnet.ae_alpha = new_ae_rate

        for ix, (X, M, Y) in enumerate(data_loader):
            X, M, Y = V(X), V(M), V(Y)

            if gpu:
                if device == 'multi':
                    X, M, Y = nn.DataParallel(
...
def ae_resnet_eval(
    ...
    assert eval_mode in [
        'val', 'test'], 'eval mode must be \'val\' or \'test\''
    ...
    curr_loss = 0
    for ix, (X, _, Y) in enumerate(data_loader):
        X, Y = V(X), V(Y)
