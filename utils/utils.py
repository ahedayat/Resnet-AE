...
def get_args():
   ...
   #  parser.add_option('-d', '--device',type='choice', action='store', dest='device',
   #                      choices=('cpu','multi',*[ 'cuda:{}'.format(ix) for ix in range(torch.cuda.device_count()) ]), default='cpu', help='optimization method' )
    ...
    parser.add_option('--ae-layers', dest='ae_layers', default=';;;;', type='string',
                      help='AE layers: True/False;[layer_id],...[layer_id];[layer_id],...[layer_id];[layer_id],...[layer_id];[layer_id],...[layer_id]')
    parser.add_option('--ae-rate', dest='ae_rate', default=0.,
                      type='float', help='curriculum rate')
